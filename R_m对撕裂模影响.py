...
#print(D[idx])
cnt=0
for cnt in np.arange(0,19,1):
    print("%.2f"%Rmz[cnt],w[cnt])
    cnt = cnt +1
#绘图

#h = plt.figure(figsize=(8, 5), dpi=120)
#plt.plot(Rmz, w/1000, 'b.' , markersize=3)
#plt.ylabel(r"$\omega_i$ [$\times 10^3$ rad/s]",fontsize = 10)
#plt.xlabel('$R_m$',fontsize = 24,color='b')
#plt.legend([r'$\omega_{i}$'], loc=0)
#plt.title(r'$R_m$---$\omega$')
#plt.show()
